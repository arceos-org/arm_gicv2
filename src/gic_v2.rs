...
impl GicDistributor {
    ...
    pub fn cpu_num() -> usize {
        unsafe{ ((GICD.TYPER.get() as usize >> 5) & 0b111) + 1 }
    }

    /// The maximum number of interrupts that the GIC supports
    pub fn max_irqs() -> usize {
        unsafe { ((GICD.TYPER.get() as usize & 0b11111) + 1) * 32 }
    }
    ...
    pub fn get_priority(int_id: usize) -> usize {
        let idx = (int_id * 8) / 32;
        let off = (int_id * 8) % 32;
        unsafe { ((GICD.IPRIORITYR[idx].get() >> off) & 0xff) as usize }
    }
    ...
    pub fn init() {
        ...
        for i in (0..max_irqs).step_by(32) {
            unsafe{ GICD.ICENABLER[i / 32].set(u32::MAX); }
        ...
        if Self::cpu_num() > 1 {
            for i in (SPI_RANGE.start..max_irqs).step_by(4) {
                ...
                unsafe{ GICD.IPRIORITYR[i / 4].set(u32::MAX); }
            }
        }
...
impl GicCpuInterface {
    ...
    pub fn set_eoi(iar: u32) {
        unsafe { GICC.EOIR.get() }
    }
